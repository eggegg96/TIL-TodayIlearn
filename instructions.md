...
하나의 커밋에 여러 작업이 들어가 있으면 나중에 revert를 해야하는 경우처럼 곤란한 상황이 생길 수 있다.

커밋 메시지들도 작성하는 방식들, 컨벤션들이 존재한다. (Git 커밋 메시지 컨벤션으로 검색하면 어러가지 나옴)

1.메세지 앞에다가 대략 뭐하는 커밋인지 적어둔다.
새로운 기능이니까 feature을 쓰고 설명을 적는다.
ex) 
...
git push 리모트 레포지토리의 master 브랜치에 내용을 전송

git pull 리모트 레포지토리의 master 브랜치의 내용을 가져온다

git commit --amend 마지막 커밋에다가 지금 작업을 추가한다.

git stash 임시 공간에 잠시 저장한다
...
충돌이 난 경우 두가지 방법 중 하나를 선택 가능하다.
1. 현재 커밋과 병합하고 싶은 커밋을 미리 내 브랜치에서 병합해서 병합 커밋을 만들고 이를 반영시킨다.
...
pull request - 협력자에게 브랜치 병합을 요청하는 메세지를 보내는 것
pull request로 병합 시에도 먼저 베이스 브랜치와 비교 브랜치(베이스 브랜치에 반영시키고 싶은 브랜치)를 설정해야 한다.

패치 Fetch - 새로운 이력을 업데이트 하는 것

ㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡ

원격 저장소의 커밋 내용과 내 컴퓨터 버전과 일치시키려면 내 컴퓨터의 브랜치로 체크아웃한 이후 pull.

ㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡ

fork - 남의 원본저장소를 내 계정의 원격저장소로 복사해오는 명령어

기본적으로 원본저장소에 커밋을 직접 퓌할 수 있는 사람은 원본저장소를 만든 소유자뿐이다.
다른 사람이 이곳에 푸시하려면 원본저장소의 소유자가 이 사람을 협력자로 등록해야하고, 등록되지 않은 사람이 이곳에 푸시를 하면 에러가 난다.

협력자 등록은 원본저장소 메ㅠㄴ에서 [Setting - Collaborators] 페이지에 들어가서 등록하려는 유저를 찾고 [Add collaborator]를 클릭한다.

원본저장소의 이력을 보려면 따로 주소를 추가해야 한다.

SourceTree에서는 클론으로 원본저장소의 주소를 입력하면 된다.

ㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡ

rebase - 이력을 조작하는 명령어이다.

ㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡ

amend - 마지막 커밋 수정하기
커밋 옵션에서 마지막 커밋 정정을 선택하면 된다.
파일의 코드를 수정하거나 커밋 메세지를 수정할 수 있다.

cherry-pick - 원하는 커밋만 떼서 현재 브랜치에 붙인다.
베이스가 되는 브랜치에서 다른 브랜치에 있는 원하는 커밋에 우클릭 후 체리픽을 실행한다.

reset - 옛날 커밋으로 되돌린다. SourceTree에서 되돌리고 싶은 지점의 커밋을 우클릭하고 이 커밋까지 현재 브랜치를 초기화에서 옵션을 선택하면 된다.
Soft - 변경사항이 스테이지 위에 올라와 있고 원하는 커밋으로 이력을 되돌린다.
Mixed - 변경사항이 스테이지 아래에 내려와 있고 원하는 커밋으로 이력을 되돌린다.
Hard - 원하는 커밋으로 이력을 되돌리고 변경사항을 폐기한다.

revert 커밋의 변경사항을 명시적으로 되돌린다. 말 그대로 명시적으로 되돌리기 때문에 이력이 남는다.

stash - 변경 사항을 잠시 따로 저장해둔다.
변경된 사항을 커밋하기 이전 stash를 사용하면 임시 공간에 저장되고 작업공간이 비워진다. 변경된 파일에 신경쓰지 않고 자유롭게 pull이나 브랜치 이동 같은 다른 작업들을 할 수 있게된다. 